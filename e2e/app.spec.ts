...
test('approaches the instructed intersection only while the vehicle moves', async ({ page }) => {
  ...
  expect(await intersection.evaluate((element) => element.parentElement?.classList.contains('road-world'))).toBe(false)
  const trafficLight = page.getByTestId('traffic-light')
  expect(Number(await trafficLight.getAttribute('data-pole-base'))).toBe(Number(await intersection.getAttribute('data-far-edge')))
...
test('uses left and right steering keys for lane changes and edge turns', async ({ page }) => {
  ...
  await expect.poll(async () => Number(await page.getByTestId('lane-camera').getAttribute('data-lane-position'))).toBe(1)
  await page.keyboard.down('ArrowUp')
  ...
  await expect.poll(async () => Number(await page.getByTestId('lane-camera').getAttribute('data-lane-position'))).toBe(0)
...
test.describe('mobile controls', () => {
  ...
  test('starts and exposes touch-sized drive controls', async ({ page }) => {
    ...
    await expect(page.getByRole('status')).toHaveText(/Changing one lane right.*Right lane/)
    await expect(page.getByLabel('Current lane')).toHaveText(/Right/)
    await expect.poll(async () => Number(await page.getByTestId('lane-camera').getAttribute('data-lane-position'))).toBeGreaterThan(0)
    await expect.poll(async () => Number(await page.getByTestId('lane-camera').getAttribute('data-lane-position'))).toBe(1)
    await expect(page.locator('.steering-wheel')).toHaveAttribute('data-angle', '0.0')
