...
test.describe('mobile controls', () => {
  ...
  test('starts and exposes touch-sized drive controls', async ({ page }) => {
    ...
    await expect(page.locator('.steering-wheel')).toHaveClass(/steering-wheel-lane-right/)
    ...
    await expect(page.locator('.steering-wheel')).toHaveClass(/steering-wheel-lane-left/)
    await page.waitForTimeout(950)
    await expect(page.locator('.steering-wheel')).not.toHaveClass(/steering-wheel-lane-left/)
    await expect(page.locator('.steering-wheel')).toHaveAttribute('style', /rotate\(0deg\)/)
  })
})
